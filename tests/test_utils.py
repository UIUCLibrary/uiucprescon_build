...
@pytest.mark.skipif(sys.platform != "win32", reason="This test is specific to Windows case insensitivity")
def test_set_env_var_keep_in_sensitive_on_window():
    """Test that set_env_var keeps environment variables case-insensitive on Windows."""
    original_env = os.environ.copy()
    test_env = {"TEST_VAR": "test_value"}
...
@pytest.mark.skipif(
    sys.platform != "win32",
    reason="This test is to make sure that ProgramFiles env var is still is accessible after using set_env_var"
)
...
def test_set_env_var_does_not_make_program_files_impossible_to_get(variable):
    test_env = {"TEST_VAR": "test_value"}
    assert variable in os.environ, f"{variable} should already be in the environment"
    with utils.set_env_var(test_env):
        pass
